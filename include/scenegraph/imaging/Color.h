...
inline Color ColorMakeWithVector4(const Vector4& v) {
...
inline Vector4 ColorToVector4(Color c) {
	constexpr float s = 1.0f / 255.0f;
	return { c.rgba.r * s, c.rgba.g * s, c.rgba.b * s, c.rgba.a * s };
}

inline Color ColorMultiply(Color a, Color b) {
	return {
		.rgba = {
...
inline Color ColorScale(Color a, uint8_t s) {
	return {
		.rgba = {
...
inline FloatColor FloatColorMakeWithVector4(const Vector4& v) {
	return {
		Clamp(v.x, 0, 1),
		Clamp(v.y, 0, 1),
		Clamp(v.z, 0, 1),
...
inline FloatColor FloatColorMakeWithColor(Color c) {
	auto v = ColorToVector4(c);
	return { v.x, v.y, v.z, v.w };
}

inline Vector4 FloatColorToVector4(const FloatColor& c) { return { c.r, c.g, c.b, c.a }; }

inline FloatColor FloatColorMultiply(FloatColor a, FloatColor b) {
	return {
		a.r * b.r,
		a.g * b.g,
		a.b * b.b,
...
inline FloatColor FloatColorScale(const FloatColor& c, float s) {
	s = Clamp(s, 0, 1);
